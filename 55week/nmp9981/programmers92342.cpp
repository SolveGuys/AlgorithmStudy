...
//더 낮은 점수
bool LowerScore(vector<int> answer,vector<int> newBow){
    for(int i=answer.size()-1;i>=0;i--){
        if(answer[i] < newBow[i]) return true;
        else if(answer[i]>newBow[i]) break;
    }
    return false;
}
//점수 세기
void Judge(int n,vector<int> getScore,vector<int> apach){
    ...
    if(arrow<=n && diff>=maxDiff && diff>=1){//화살 개수 충분하고 더 큰 점수차가 나오면
        ...
        //동점인 경우
        if(diff == maxDiff){
            if(LowerScore(answer,bow)==true) answer = bow;
        }else{
